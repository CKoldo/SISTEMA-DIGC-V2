...
export function Administracion() {
  ...
  const guardarEsinad = () => {
    ...
    const cronogramaActual = cronogramas.find(c => c.id === editandoCronogramaId);
    if (!cronogramaActual) return;

    const cronogramaActualizado = {
      ...cronogramaActual,
      rangos: cronogramaActual.rangos.map(r =>
        r.id === editandoRangoId ? { ...r, esinadId: nuevoEsinadId.trim() } : r
      ),
    };

    const nuevoCronogramas = cronogramas.map(c => c.id === editandoCronogramaId ? cronogramaActualizado : c);
    setCronogramas(nuevoCronogramas);
    localStorage.setItem('cronogramas', JSON.stringify(nuevoCronogramas));
  ...
  const guardarEsinadParaTodos = () => {
    ...
    const esinadLimpio = esinidParaTodos.trim();
    let contadorActualizados = 0;

    const nuevoCronogramas = cronogramas.map(cronograma => {
      const rangosActualizados = cronograma.rangos.map(rango => {
        // Verificar si este rango está en los filtrados
        const estaEnFiltrados = rangosFiltrales.some(r => r.id === rango.id && r.cronogramaId === cronograma.id);
        if (estaEnFiltrados && !rango.esinadId) {
          contadorActualizados++;
          return { ...rango, esinadId: esinadLimpio };
        }
        return rango;
      });
  ...
  const eliminarEsinad = () => {
    ...
    const cronogramaActualizado = {
      ...cronogramaActual,
      rangos: cronogramaActual.rangos.map(r =>
        r.id === editandoRangoId ? { ...r, esinadId: undefined } : r
      ),
    };
