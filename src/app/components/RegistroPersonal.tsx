...
import { ArrowLeft, UserPlus, Trash2, CalendarDays, Download } from 'lucide-react';
...
export function RegistroPersonal() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.nombre || !formData.apellido || !formData.fechaIngreso || !formData.email || !formData.puesto) {
    ...
    toast.success(`Personal ${formData.nombre} ${formData.apellido} registrado exitosamente`);
  ...
  const exportarPersonalJSON = () => {
    ...
    saveAs(blob, `SISTEMA_DIGC_Personal_${new Date().toISOString().split('T')[0]}.json`);
  ...
  const exportarVacacionesJSON = () => {
    ...
    const blob = new Blob([JSON.stringify(cronogramas, null, 2)], { type: 'application/json' });
    saveAs(blob, `SISTEMA_DIGC_Vacaciones_${new Date().toISOString().split('T')[0]}.json`);
    toast.success('Vacaciones exportadas a JSON');
  };

  const handleImportExcelPersonal = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        ...
        const nuevos: any[] = rows.map((r, idx) => {
          ...
          const nombre = get(['nombre', 'Nombre', 'NOMBRE', 'firstName', 'FirstName']);
          const apellido = get(['apellido', 'Apellido', 'APELLIDO', 'lastName', 'LastName']);
          const fechaIngreso = get(['fechaIngreso', 'fecha_ingreso', 'FechaIngreso', 'Fecha de Ingreso', 'fecha de ingreso']);
          const email = get(['email', 'Email', 'EMAIL']);
          const puesto = get(['puesto', 'Puesto', 'cargo', 'Cargo']);
          const id = get(['id', 'ID']) || `${Date.now()}-${idx}`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="ml-auto flex gap-2">
              <Button onClick={exportarPersonalJSON} variant="outline" className="gap-2">
                <Download className="w-4 h-4" />
                Exportar Personal (JSON)
              </Button>
              <Button onClick={exportarVacacionesJSON} variant="outline" className="gap-2">
                <Download className="w-4 h-4" />
                Exportar Vacaciones (JSON)
              </Button>
              <Button onClick={() => fileExcelRef.current?.click()} variant="outline" className="gap-2">
                <Download className="w-4 h-4" />
                Importar Personal (Excel)
              </Button>
              <Button onClick={limpiarDatos} variant="destructive" className="gap-2">
                Limpiar Datos
              </Button>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <input ref={fileExcelRef} type="file" accept=".xlsx, .xls" onChange={handleImportExcelPersonal} style={{ display: 'none' }} />
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {personal.length} {personal.length === 1 ? 'empleado registrado' : 'empleados registrados'}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {personal.length === 0 ? (
                    ...
                    <TableBody>
                      {personal.map((p) => (
                        <TableRow key={p.id}>
                          ...
                          <TableCell>{formatearFecha(p.fechaIngreso)}</TableCell>
