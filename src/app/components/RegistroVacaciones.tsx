...
import { ArrowLeft, CalendarPlus, AlertTriangle, CheckCircle, Trash2, Info } from 'lucide-react';
...
import { format, parseISO, differenceInDays, addDays, isFriday } from 'date-fns';
...
export function RegistroVacaciones() {
  ...
  const [selectedCronograma, setSelectedCronograma] = useState<CronogramaVacaciones | null>(null);
  ...
  useEffect(() => {
    // Cargar personal y cronogramas desde localStorage
    const savedPersonal = localStorage.getItem('personal');
    if (savedPersonal) {
      setPersonal(JSON.parse(savedPersonal));
    }

    const savedCronogramas = localStorage.getItem('cronogramas');
    if (savedCronogramas) {
      setCronogramas(JSON.parse(savedCronogramas));
    }
  }, []);

  useEffect(() => {
    // Cuando se selecciona un personal, buscar o crear su cronograma
    if (selectedPersonalId) {
      ...
      if (empleado) {
        ...
        if (!cronograma) {
          ...
          localStorage.setItem('cronogramas', JSON.stringify(newCronogramas));
        }
  ...
  useEffect(() => {
    ...
    if (fechaInicio) {
      ...
      if (isFriday(inicio)) {
        ...
        toast.info('Se seleccionó un viernes. La fecha fin se estableció automáticamente al domingo.');
      }
    }
  }, [fechaInicio]);
  ...
  const handleAgregarRango = () => {
    ...
    const esFlexible = diasSolicitados <= 7 && selectedCronograma.diasFlexiblesUsados < 7;
    ...
    const cronogramaActualizado: CronogramaVacaciones = {
      ...selectedCronograma,
      rangos: [...selectedCronograma.rangos, nuevoRango],
    ...
    localStorage.setItem('cronogramas', JSON.stringify(cronogramasActualizados));
  ...
  const handleEliminarRango = (rangoId: string) => {
    ...
    const nuevosRangos = selectedCronograma.rangos.filter((r) => r.id !== rangoId);
    ...
    if (rango.tipo === 'flexible') {
      diasFlexiblesUsados = selectedCronograma.diasFlexiblesUsados - rango.diasSolicitados;
    } else {
      diasBloqueUsados = selectedCronograma.diasBloqueUsados - rango.diasSolicitados;
    }
  ...
  const handleCronogramaActualizado = (cronogramaActualizado: CronogramaVacaciones) => {
    const cronogramasActualizados = cronogramas.map((c) =>
      c.id === cronogramaActualizado.id ? cronogramaActualizado : c
    );
    setCronogramas(cronogramasActualizados);
  ...
  const diasDisponibles = selectedCronograma ? calcularDiasDisponibles(selectedCronograma) : null;
  const empleadoSeleccionado = personal.find((p) => p.id === selectedPersonalId);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      <div className="space-y-1">
                        <p><strong>Periodo vacacional:</strong> {selectedCronograma.anioVacacional}</p>
                        <p><strong>Desde:</strong> {format(parseISO(selectedCronograma.fechaInicioAnio), 'dd/MM/yyyy', { locale: es })}</p>
                        <p><strong>Hasta:</strong> {format(parseISO(selectedCronograma.fechaFinAnio), 'dd/MM/yyyy', { locale: es })}</p>
                      </div>
                    </AlertDescription>
                  </Alert>
                )}
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        min={fechaInicio || selectedCronograma.fechaInicioAnio.split('T')[0]}
                        max={selectedCronograma.fechaFinAnio.split('T')[0]}
                      />
                    </div>
                  </div>
                  ...
                  {fechaInicio && fechaFin && (
                    <div className="p-3 bg-blue-50 rounded-md text-sm">
                      <p className="font-medium text-blue-900">
                        Días solicitados: {differenceInDays(parseISO(fechaFin), parseISO(fechaInicio)) + 1}
                      </p>
                    </div>
                  )}
            ...
            {/* Rangos Programados */}
            {selectedCronograma && selectedCronograma.rangos.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle>Vacaciones Programadas</CardTitle>
                  <CardDescription>
                    {selectedCronograma.rangos.filter(r => (r.estado ?? 'activo') === 'activo').length} {selectedCronograma.rangos.filter(r => (r.estado ?? 'activo') === 'activo').length === 1 ? 'periodo programado' : 'periodos programados'}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {selectedCronograma.rangos.filter(r => (r.estado ?? 'activo') === 'activo').map((rango) => {
  const linkedOriginals = rango.reprogramadoDesde && rango.reprogramadoDesde.length > 0
    ? rango.reprogramadoDesde.map(id => selectedCronograma.rangos.find(r => r.id === id)).filter(Boolean)
    : [];

  return (
    <div id={`rango-${rango.id}`} key={rango.id} className="flex items-start justify-between p-4 bg-gray-50 rounded-lg">
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-2">
            ...
            {rango.estado === 'reprogramado' && (
              <Badge variant="outline" className="ml-2">
                ...
                {linkedOriginals.length > 0 && (
                  <span className="ml-2">de {linkedOriginals.map((o, idx) => (
                    <button
                      key={o!.id}
                      onClick={() => {
                        const el = document.getElementById(`rango-${o!.id}`);
                        if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                      }}
                      className="underline ml-1 text-sm text-indigo-700"
                      type="button"
                    >
                      {formatearRangoFechas(o!.fechaInicio, o!.fechaFin)}{idx < linkedOriginals.length - 1 ? ', ' : ''}
                    </button>
                  ))}</span>
                )}
              </Badge>
          ...
          {rango.incluye_finde && (
            <Badge variant="outline">Incluye fin de semana</Badge>
          )}
        </div>
        <p className="font-medium text-gray-900">
          {formatearRangoFechas(rango.fechaInicio, rango.fechaFin)}
        </p>
        <p className="text-xs text-gray-500 mt-1">ID: {rango.id}</p>
        <p className="text-sm text-gray-600">
          {rango.diasSolicitados} {rango.diasSolicitados === 1 ? 'día' : 'días'}
        </p>
      </div>
      <Button
        variant="ghost"
        ...
        className={rango.estado === 'reprogramado' ? 'text-gray-400 cursor-not-allowed' : 'text-red-600 hover:text-red-700 hover:bg-red-50'}
            ...
            {diasDisponibles && empleadoSeleccionado && selectedCronograma && (
              <>
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-gray-500">de 30 días totales</p>
                    </div>

                    <div className="pt-4 border-t space-y-3">
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="text-sm font-medium text-gray-900">Días Flexibles</p>
                          <p className="text-xs text-gray-500">Rangos de 1-7 días</p>
                        </div>
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="text-sm font-medium text-gray-900">Días en Bloque</p>
                          <p className="text-xs text-gray-500">Rangos de 7+ días</p>
                        </div>
                        <div className="text-right">
                ...
                <Card className="bg-indigo-50 border-indigo-200">
                  <CardHeader>
                    <CardTitle className="text-indigo-900 text-sm">Recordatorios</CardTitle>
                  </CardHeader>
                  <CardContent className="text-xs text-indigo-800 space-y-2">
                    ...
                    <div className="flex gap-2">
                      <CheckCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                      <p>Los días en bloque requieren mínimo 7 días consecutivos</p>
                    </div>
                    <div className="flex gap-2">
                      <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                      <p>Seleccionar un viernes incluye automáticamente sábado y domingo</p>
                    </div>
                  </CardContent>
                </Card>
              </>
            ...
            {!selectedPersonalId && (
              <Card>
                <CardContent className="pt-6 text-center text-gray-500">
                  ...
                  <p className="text-sm">Seleccione un empleado para comenzar</p>
                </CardContent>
              </Card>
            )}
          </div>
